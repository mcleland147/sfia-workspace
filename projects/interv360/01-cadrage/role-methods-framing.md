...
- quels éléments devront être répercutés dans les futurs fichiers `docs/methods/roles/` ?
...
À chaque nouveau livrable Interv360, il faut vérifier si `role-methods-framing.md` ou les futurs fichiers `docs/methods/roles/` doivent être mis à jour. Si un rôle a produit une **valeur projet** ou une **valeur SFIA** nouvelle, cette contribution doit être capitalisée.
...
**Emplacement cible proposé :** `docs/methods/roles/`
...
| Prompt découpage templates par rôle | Extraire un fichier `docs/methods/roles/*.md` depuis ce document | Prompt |
...
La prochaine étape pourra consister à **découper ce document** en fichiers méthodologiques par rôle dans `docs/methods/roles/`, puis à alimenter les référentiels SFIA (templates, prompts, standards) sans reproduire une description RH générique des rôles.
