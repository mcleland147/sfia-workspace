...
export class GetLivingProjectStateVersion {
  constructor(
  ...
  async execute(input: {
    ...
    if (!Number.isInteger(input.version) || input.version < 1) {
      return {
        ok: false,
        error: createProjectError({
          detailCode: "LPS_INVALID",
          timestamp,
          correlationId,
          projectId: input.projectId,
          internalCauseRef: "version_invalid",
        }),
      };
    }
    ...
    try {
      ...
      const livingProjectState = await this.lps.findByProjectAndVersion(
        input.projectId,
        input.version,
      );
      if (!livingProjectState) {
        ...
        return {
          ...
          error: createProjectError({
            ...
            correlationId,
            projectId: input.projectId,
            internalCauseRef: "missing_lps_version",
          }),
        };
      }
