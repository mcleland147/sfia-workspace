# ChatGPT Review Pack — Pre-MW2 OpenAI-Native-First Documentary Truth-Sync
...
| **Base main** | `5030b0e8c489754b43b9044f9e3f738e49a8bad5` |
| **Branch** | `docs/sfia-studio-nora-openai-native-first-truth-sync` |
| **Local working tree** | UNKNOWN — no local-state claim |
| **Purpose** | Capitaliser avant MW2 les décisions/trajectoires transverses Nora MW2→MW6 : OpenAI-native-first, frontière Studio/Nora/OpenAI, GPT-5.6/model-reasoning trajectory, future comparative campaigns, model-routing anti-claim |
...
1. `projects/sfia-studio/convergence/sfia-studio-convergence-build-doctrine.md` @ base `5030b0e8…`
2. `projects/sfia-studio/convergence/sfia-studio-convergence-roadmap.md` @ base `5030b0e8…`
3. `projects/sfia-studio/nora-cognitive-completion/05-nora-source-locked-cognitive-backlog.md`
4. `projects/sfia-studio/nora-cognitive-completion/06-nora-mw0-versioned-cognitive-eval-harness-readiness.md`
...
Provider facts are **revalidable current inputs**, not permanent SFIA doctrine.

### Morris decisions / trajectory instructions consumed in this cycle

- Separate **documentary cross-wave truth-sync** before MW2 Readiness/Requalification.
- After documentary integration: requalify MW2, review/validate qualification with Morris, then only a distinct MW2 Delivery GO.
- **NORA OPENAI-NATIVE-FIRST:** before developing a cognitive solution internally, evaluate relevant native OpenAI capabilities/services; prefer native usage/adaptation where the same applicable conditions are satisfied.
- Preserve boundary: Studio = business/product/governance/authority model; Pilote = runtime human decision/confirmation actor; Nora = cognitive layer; OpenAI = cognitive primitives/provider layer.
- GPT-5.6 is the current Nora REAL evidence family; MW0/MW1 REAL used `gpt-5.6-luna` with `reasoning.effort=none` according to consumed MW0/MW1 campaign evidence/handoff provided in the qualification context.
...
Reason: source-locked story semantics remain unchanged; no MW2-S03; no AC/DAG rewrite.

`projects/sfia-studio/nora-cognitive-completion/06-nora-mw0-versioned-cognitive-eval-harness-readiness.md`

Reason: historical Morris-validated readiness contract already contains D0/R1/R2/R3, provider capability revalidation, model/reasoning experiment semantics and MW1→MW6 reuse. Post-Delivery/current trajectory is capitalized in doc08 rather than retroactively mutating a historical validated readiness document.

`projects/sfia-studio/convergence/sfia-studio-convergence-roadmap.md`

Reason: current Roadmap already carries the factual gate `MW1 CLOSED → MW2 Readiness/Requalification`, MW2 NOT AUTHORIZED, Production model routing NOT SELECTED. To avoid a high-risk monolithic rewrite/duplication of its large living body, doc08 is introduced as the dedicated Nora cross-wave trajectory surface and is explicitly bound into the Build Doctrine metadata/source governance/pre-check. The Roadmap remains state/gate truth; doc08 carries the cognitive construction/model-reasoning trajectory.

---

## 3. Complete content — CREATED doc08
...
## 4. Useful modified content — Build Doctrine
...
## 5. Useful modified content — OD-04 / doc07
...
6. MW0 Option C regression/parity gate — PASS WITH NON-BLOCKING RESERVES / DETERMINISTIC ONLY
7. MW1-S01/S02/S03 Delivery — INTEGRATED / POST-MERGE VERIFIED (PR #450)
8. MW1 deterministic proof — PASS
9. MW1 REAL boundary — PROVEN · campaign mw1-real-boundary-corr01-1788253662383
...
```text
1. identify desired cognitive outcome and SFIA-specific boundary
2. perform OpenAI Capability Fit Check
...
7. prove claims through MW0 D0/R1/R2/R3 as applicable
8. no homegrown generic equivalent by default when mature native primitive fits
9. do not alter source-locked C5 semantics
```
...
- No change to C5 source-locked backlog.
- No new MW2 story.
...
- OD-04 remains CONSUMED and unchanged in architecture selection.
- Option C preserved.
...
- Studio owns product/business/authority model.
...
- GPT-5.6 is recorded as current REAL evidence family / current provider snapshot, not permanent doctrine.
- Luna+none recorded as MW0/MW1 proven starting point, not final production model selection.
- Effective F1 runtime configuration remains TO VERIFY through `PRE-MW2-MODEL-BASELINE-01`.
...
- MW0 historical readiness remains unmodified and continues to own D0/R1/R2/R3 contract.
...
`RES-DOC-ROADMAP-01` — The large primary Convergence Roadmap file is not directly modified in this branch. Its current facts remain compatible (`MW1 CLOSED`, `MW2 next`, `MW2 NOT AUTHORIZED`, `Production model routing NOT SELECTED`). The new doc08 is bound directly from Build Doctrine metadata/A11 and the Nora cognitive pre-check rule, which ensures future Nora qualification discovers it after integration. A future compact Roadmap maintenance may add a direct pointer if useful, but no duplicate trajectory body is required.
...
- GPT-5.6 prerequisite if effective runtime is not aligned.
- Plan model/reasoning comparative evaluation with per-wave QA trigger + post-MW6 default global campaign.
...
- `OPENAI-NATIVE-FIRST` — captured.
- mandatory OpenAI Capability Fit Check MW2→MW6 — captured.
- Studio/Nora/Pilote/OpenAI boundary — captured.
- GPT-5.6 current REAL evidence family — captured as revalidable truth, not doctrine.
- Luna + none MW0/MW1 starting evidence — captured, not production routing decision.
- `PRE-MW2-MODEL-BASELINE-01` — captured.
- MW2 reasoning evaluation — captured.
- future model × reasoning campaign — captured with early QA trigger / post-MW6 default.
- Production model routing — NOT SELECTED.
- C5 source-lock — unchanged.
- MW0 historical readiness — unchanged.
- OD-04 — preserved; MW1 stale forward truth corrected.
- Build Doctrine — R22 amendment candidate added.
- runtime/code — zero change.
- MW2 — NOT AUTHORIZED / NOT STARTED.
- runtime v3 — NON ADOPTED.
