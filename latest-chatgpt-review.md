# ChatGPT Review Pack — SFIA Studio Cycle 7 Cursor Real Spike Critical

## 0. Métadonnées

| Champ | Valeur |
|-------|--------|
| **Date / heure** | 2026-07-19 17:48:48 CEST (Europe/Paris) |
| **Cycle** | 7 — Intégration / DevOps |
| **Profil** | Critical |
| **Décision Morris** | Spike transmission réelle bornée vers Cursor |
| **Branche** | `spike/sfia-studio-cursor-real-adapter` (**locale**) |
| **HEAD / origin/main** | `1c44bc01ff115216d515dfdd9bde5d7eaccfa966` |
| **PR #224** | MERGED |
| **Handoff précédent** | `c6dd666` Cycle 13 PR readiness |
| **Niveau** | **full** |
...
**PASS** — main @ `1c44bc0…` ; PR #224 MERGED ; delivery remote absente ; harness présent.

État courant (non commité) :
...
Cycle 13 Critical ; PR #224 ; commit `e1040b5…` ; Cursor fixture ; Cursor réel gate fermé avant ce GO ; UI fermée ; Docker conditionnel ; POC non réussi.
...
- PATH `cursor` : absent
- App bin `cursor` 3.12.17 : présent
- `cursor agent --help` : CLI Agent documentée (`--print`, `--mode ask|plan`, `--workspace`, …)
- Effet de bord : auto-install `~/.local/bin/cursor-agent` (2026.07.16-899851b)
- Auth : **Not logged in** ; `CURSOR_API_KEY` absent
- `@cursor/sdk` 1.0.23 sur npm : **non installé** (dépendance évitée)

### Auth status (no secrets)
...
Candidat : **CLI `cursor agent`** via chemin absolu app.  
Statut : officiel help local · expérimental harness · non production.

Voir matrice complète dans `29`.

---

## 5. Architecture / feature flag

- Fixture = défaut stable (préservée)
- `CursorExecutorPortRealSpike` expérimental
- Flags : `SFIA_CURSOR_REAL_SPIKE=1` + `enabled:true` ; live test `SFIA_CURSOR_REAL_LIVE=1`
- `shell:false` ; cwd sandbox ; env filtré ; timeout/STOP wrapper

---

## 6. Exécution réelle

Processus **invoqué** ; **échec auth** ; exit 1 ; aucun `output/summary.txt`.

```json
{
  "ok": false,
  "summary": "real-spike failed exit=1 timedOut=false",
  "artifacts": [],
  "errorCode": "CURSOR_SPIKE_FAILED",
  "durationMs": 1261,
...
| 04 | PROUVÉ wrapper |
| 05 | PROUVÉ |
| 06 | PROUVÉ (pas de write agent) |
| 07 | PROUVÉ harness |
| 08 | PROUVÉ wrapper ; CLI STOP NON PROUVÉ |
...
Non requis pour découverte ; non introduit. Recommandation : attendre run nominal natif.
...
export interface CursorRealSpikeOptions {
  ...
  /** Force agent mode: ask|plan (read-oriented). Default ask. */
  agentMode?: "ask" | "plan";
}
...
export function buildCursorAgentArgv(input: {
  ...
  const base = input.useNestedAgentSubcommand
    ? ["agent", "--print", "--mode", input.mode, "--workspace", input.workspace, "--trust", input.instruction]
    : ["--print", "--mode", input.mode, "--workspace", input.workspace, "--trust", input.instruction];
  return { command: input.bin, argv: base };
}
...
export class CursorExecutorPortRealSpike implements CursorExecutorPort {
  ...
  async executeSpikeContract(contract: CursorRealSpikeContract): Promise<CursorRealSpikeResult> {
    ...
    const mode = this.opts.agentMode ?? "ask";
...
describe("cursor real spike — live CLI (opt-in)", () => {
  ...
  it.runIf(live)("invokes cursor agent in ask mode against sandbox", async () => {
    ...
    const result = await spike.executeSpikeContract(sampleContract({ timeoutMs: 60_000 }));
    expect(result.realCursorProcessInvoked).toBe(true);
    expect(result.productionReadyClaimed).toBe(false);
    // Auth may fail; still captures exit metadata
...
```
{
  "spikeId": "spike-cur-live-001",
  "requestId": "req-spike-live-001",
  "executionId": "exec-1784476022824",
  "mechanismId": "cursor-agent-cli",
  "mechanismVersion": "observed-local",
  ...
  "cursorMode": "real-spike",
  "initiatedBy": "Morris",
  "initiatedAt": "2026-07-19T15:47:02.824Z",
  "contractHash": "c21a28d72d26c3cb6deeda808354001fc19c32f38a5d1858ce601661c3f4008d"
}
```
### `projects/sfia-studio/harness/spikes/cursor-real/sandbox/proofs/git-diff-before.txt`
...
./proofs/contract.json 3041515738
./proofs/git-diff-before.txt 4294967295
./proofs/git-diff-post-run.txt 4294967295
./proofs/git-diff-pre-run.txt 4294967295
./proofs/git-status-before.txt 2208561245
./proofs/git-status-post-run.txt 1423255276
./proofs/git-status-pre-run.txt 1423255276
./proofs/result.json 1263448397
./proofs/run-spike-console.txt 4294967295
./proofs/sandbox-listing-before.txt 3252486968
./proofs/sandbox-listing-pre-run.txt 1163463270
...
## 14. Contenu + diffs — modifiés

### `projects/sfia-studio/README.md` — contenu
...
### `projects/sfia-studio/README.md` — diff
...
+| [29-poc-cursor-real-spike-report.md](./29-poc-cursor-real-spike-report.md) | Spike Cursor réel (Cycle 7) |
 ...
 ### Navigation backlog POC
```
### `projects/sfia-studio/07-product-trajectory-and-decision-pack.md` — contenu
...
| D-NEXT-30 | UI app/** / Cursor réel / Docker | Morris | UI/Docker **fermés** ; spike Cursor **inconclusive** (`29`) |
...
### `projects/sfia-studio/07-product-trajectory-and-decision-pack.md` — diff
...
+| D-NEXT-30 | UI app/** / Cursor réel / Docker | Morris | UI/Docker **fermés** ; spike Cursor **inconclusive** (`29`) |
 ...
 ---
```
### `projects/sfia-studio/28-poc-orchestration-delivery-gate-pack.md` — contenu
...
Spike local `spike/sfia-studio-cursor-real-adapter` : interface CLI `cursor agent` **trouvée** et **invoquée** ; authentification **requise** ; run nominal **non** démontré ; fixture stable **inchangée**. Voir `29`.

Commit/push/PR du spike : **non autorisés** ici.

```

### `projects/sfia-studio/28-poc-orchestration-delivery-gate-pack.md` — diff
...
+Commit/push/PR du spike : **non autorisés** ici.
```
### `projects/sfia-studio/harness/README.md` — contenu
...
### `projects/sfia-studio/harness/README.md` — diff
...
+- Ne jamais activer sans `SFIA_CURSOR_REAL_SPIKE=1`.
 ...
 - Allowlist détaillée toujours sujette à raffinement Morris.
```
### `projects/sfia-studio/harness/package.json` — contenu
...
### `projects/sfia-studio/harness/package.json` — diff
...
+    "spike:cursor-real": "tsx spikes/cursor-real/run-spike.ts"
   ...
   "engines": {
     "node": ">=20"
```
### `projects/sfia-studio/harness/src/index.ts` — contenu
...
### `projects/sfia-studio/harness/src/index.ts` — diff
...
## 15. Risques / réserves / dette / décisions

Voir `29` §§13–16.

Décisions Morris : auth spike ; versionnement ; SDK ; UI ; Docker ; allowlist réelle.
...
Ne conclut pas : CURSOR REAL PRODUCTION READY · POC COMPLETE · READY FOR MERGE · DOCKER VALIDATED.
