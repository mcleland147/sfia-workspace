# SFIA Fast Track

## 1. Objectif

SFIA Fast Track est la méthode de livraison rapide, contrôlée et capitalisable utilisée pour créer des applications ou produits numériques avec assistance IA.

Elle vise à combiner :

- cadrage clair ;
- cycles courts ;
- garde-fous explicites ;
- PR ciblées ;
- validation systématique ;
- documentation continue ;
- automatisation progressive.

## 2. Arborescence

```text
method/sfia-fast-track/
├── README.md
├── core/                  # Capitalisation, règles, standards, roadmap
├── automation/              # Vision application factory
├── cycles/                  # Capitalisations par cycle de livraison
├── ui/                      # Méthode Figma Design-first
├── audit-rex/               # Audits et REX projet
├── documentation/           # Qualité doc, workspace, Notion
├── templates/               # Modèles cycle, PR, audit, REX
├── checklists/              # Validation, garde-fous, publication
└── archive/                 # Documents historiques
```

## 3. Documents principaux

### Core (`core/`)

Normative reference: `docs/foundation/sfia-engineering-principles.md`

- `sfia-global-capitalization-reference.md`
- `sfia-rules-update.md`
- `sfia-architecture-standards.md`
- `sfia-consolidation-roadmap.md`
- `sfia-chatgpt-cursor-operating-model.md` — ChatGPT ↔ Cursor operating model (foundation v1.1)
- `sfia-rules-and-guardrails.md` — canonical rules and guardrails (foundation v1.1)
- `sfia-knowledge-layer.md` — knowledge layer and Documentation Routing Matrix (foundation v1.1)

### Automation (`automation/`)

Synthesis reference: `sfia-automation-architecture.md` (foundation v1.1)

Prompt catalog: `prompts/prompt-catalog.md` (candidate v1.1)

Prompt templates: `prompts/templates/` — full reusable SFIA prompt templates derived from the Prompt Catalog

- `sfia-automation-architecture.md`
- `sfia-prompt-generation-engine.md` — specification for generating SFIA-compliant Cursor prompts (foundation v1.1)
- `sfia-repository-execution-engine.md` — specification for executing SFIA-compliant Cursor prompts in the Git repository (foundation v1.1)
- `sfia-validation-engine.md` — specification for validating prompts, repository execution, diffs, commits and deliverables (foundation v1.1)

### Cycles (`cycles/`)

- `interv360-mvp-delivery-capitalization.md`
- `interv360-final-capitalization.md`

### UI (`ui/`)

- `figma-design-first-ui-method.md`

### Audit & REX (`audit-rex/`)

- `interv360-application-rex.md`

### Documentation (`documentation/`)

- `documentation/capitalization/sfia-v2.6/2026-07-11-sfia-v2.6-repository-cartography.md`
- `documentation/capitalization/sfia-v2.6/2026-07-11-sfia-v2.6-repository-standard-principles.md`
- `notion-publication-plan.md`
- `workspace-audit/` — audits et inventaires du workspace

### Templates (`templates/`)

- `cycle-template.md`
- `pr-body-template.md`
- `post-merge-template.md`
- `audit-template.md`
- `rex-template.md`
- `mvp-delivery-pattern-template.md`

### Checklists (`checklists/`)

- `sfia-validation-checklist.md` — operational validation checklist aligned with Validation Engine v1.1
- `validation-checklist.md`
- `guardrails-checklist.md`
- `notion-publication-checklist.md`

## 4. Règles clés

- un cycle = un objectif ;
- PR unique par cycle ;
- post-merge documenté ;
- audit avant cycle majeur ;
- REX après jalon majeur ;
- Figma Design-first pour UI premium ;
- Notion uniquement après plan validé ;
- backend/API dans cycle dédié ;
- automatisation progressive.

## 5. Workflow standard

1. Cadrer le besoin ;
2. Ouvrir une branche dédiée ;
3. Définir livrable et garde-fous ;
4. Exécuter le cycle ;
5. Valider diff/build/tests ;
6. Commit ;
7. PR ;
8. Merge ;
9. Post-merge ;
10. REX ou capitalisation si nécessaire.

## 6. Vision d'automatisation

La cible long terme est de transformer SFIA Fast Track en application factory assistée par IA.

Référence : `automation/sfia-automation-architecture.md`

## 7. Statut

Ce README sert de point d'entrée à la méthode SFIA Fast Track.
